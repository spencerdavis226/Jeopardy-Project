...
For fetching categories, the API responds with data like:
...
- **GET** `https://rithm-jeopardy.herokuapp.com/api/category?id=[integer]` returns questions and answers for the category.
...
To run the Jeopardy game locally:
...
1. **Fetch Categories**: The application makes an API request to fetch random categories.
2. **Populate the Game Board**: Once categories are fetched, the game board is populated with category titles.
3. **Clicking on a Clue**: When the user clicks on a clue, the corresponding question is revealed. Upon clicking again, the answer appears.
4. **Restart the Game**: The **Restart** button reloads new categories and questions, giving players a fresh start.
